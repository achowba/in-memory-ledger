...
**Double entry.** Single entry per customer account, with no contra accounts for fee income, interest expense, settlement clearing or suspense. _Defers:_ no trial balance, no way to prove value is conserved across the book rather than within one account, and finance cannot see fee income or interest expense at all. A ledger that cannot be balanced is a record, not a ledger. This is the largest cut here.
...
**Idempotency.** No event carries a key and nothing deduplicates. _Defers:_ a retried delivery double posts. In a system fed by a queue with at least once delivery, this is not an edge case. It is routine.

**Concurrency control.** Single threaded, no locking, no optimistic versioning. _Defers:_ two authorizations arriving together both read the same available balance and both approve, and the account goes overdrawn past the limit meant to prevent exactly that.

**Authorization expiry.** _Defers:_ holds leak, and customer funds are locked with no event that will ever free them.

**Reversal reason codes.** _Defers:_ the consumer protection exposure in section 2, and the one failing test.

**Tamper evidence.** The log is append only by construction, with frozen records, but it is not hash chained or signed. _Defers:_ an append only log with no tamper evidence is not an audit record, whatever the code does. Cheaper to add before there is history to migrate.
...
**Debit interest.** An overdraft is priced by a flat fee only. _Defers:_ an overdraft is mispriced, and regressively, since a small overdraft pays the same fee as a large one.

**A multi currency fee schedule.** Only AED is priced. _Defers:_ an account in another currency cannot be charged, which the code raises on rather than guessing. Honest, and still a gap.
...
**Event schema versioning.** _Defers:_ the first change to an event shape makes the historical log unreadable by the current code, which is the failure that turns an append only log from an asset into a liability.
