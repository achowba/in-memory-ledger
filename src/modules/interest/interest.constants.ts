 ...
 *
 * @remarks
 * The rate is 0.04 percent per day, which is 4 parts in 10000. It is held as a pair of
 * integers rather than as 0.0004, because 0.0004 is not representable in binary floating
 * point and a rate that is already wrong before it is applied cannot be rounded back into
 * correctness.
 *
 * An accrual is therefore `balanceMinor * 4n / 10000n`, which is exact integer arithmetic up
 * to the single division at the end.
 *
 * On the size of the rate: 0.04 percent per day is about 14.6 percent a year simple, and
 * about 15.7 percent compounded. That is a lending rate, not a deposit rate. The brief
 * supplies it, so the implementation uses it, but a rate this high on a credit balance is
 * worth flagging rather than transcribing without comment. See NUMBERS.md.
 */
 ...
 * numerator stays a whole number: expressing 0.04 percent as 1n over 2500n would also be
 * exact but would hide the rate a reader is checking against the brief.
 */
